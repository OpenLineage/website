...
In the `[dbt-ol](https://github.com/OpenLineage/OpenLineage/blob/main/integration/dbt/scripts/dbt-ol)` script, the integration uses the project’s `ParentRunMetadata` and `DbtArtifactProcessor` classes, both of which can be found in the OpenLineage common integration, to parse metadata from the dbt `manifest` and `run_result` in order to produce OpenLineage events:  
