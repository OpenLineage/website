...
image: ./image.svg
banner: ./banner.svg
description: Spark ushered in a new age of data democratization... and left us with a mess of hidden dependencies, stale datasets, and failed jobs
---
...
distributed file systems or object stores, like HDFS or S3. This had enabled us to build analytic systems that could
handle traditional, table-structured data alongside flexible, unstructured, JSON blobs, giving us access to more data
and moving faster than we’d previously been able to.

The problem was that taking the data out of the Data Warehouses meant that the people who really needed access to the
...
software engineers to build custom tools to provide access, meaning the bottleneck had moved from the systems that
...
tools to process raw data in object stores without the dependency on software engineers. While a lot of folks were
...
The approach to integrating with Spark is not super novel nor is it complicated to integrate into your own system. Spark
has had a SparkListener interface since before the 1.x days. OpenLineage integrates with Spark by implementing that
interface and collecting information about jobs that are executed inside of a Spark application. To activate the
listener, add the following property to your Spark configuration:
```
...
access to BigQuery and some GCS bucket. I added mine to a file called bq-spark-demo.json.
...
Then cd into the integration/spark directory. Run `mkdir -p notebooks/gcs` and copy your service account credentials
...
![Marquez job lineag](./marquez_job_graph.png)
