...
> **Note:** You can follow Marquez's [quickstart](https://marquezproject.github.io/marquez/quickstart.html#write-sample-lineage-metadata-to-marquez) guide to seed the HTTP API server with sample lineage metadata used in this blog post.
...
In this blog, we won't go into how to enable lineage metadata collection for Airflow DAGs. But, we encourage you to take a look at Marquez's Airflow [example](https://github.com/MarquezProject/marquez/tree/main/examples/airflow) to learn how to troubleshoot failing DAGs using Marquez.
...
Then, using the script `backfill.sh`, we automatically backfill all DAGs downstream of `example.etl_orders`:
...
In this blog post, we showed how easy it can be to automate backfilling DAGs downstream of a data quality issue using lineage metadata. With only two steps, we fixed the weekly food order trends report!
