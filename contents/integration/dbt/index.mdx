...
Next, we specify where we want dbt to send OpenLineage events. `dbt-ol` will use the `OPENLINEAGE_URL` environment variable to know where to send OpenLineage events. For example, to send OpenLineage events to a local instance of Marquez, use:

```bash
OPENLINEAGE_URL=http://localhost:5000
```

Finally, we need to specify a namespace where the lineage events will be stored. For example, to use the namespace "dev":
